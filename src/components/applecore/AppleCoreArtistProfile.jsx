...
export default function AppleCoreArtistProfile({ artistId, onSelectArtist }) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const rank = computePlatformRank(allArtists, activeProfile.id, "applecore");
        setPlatformRank(rank);
        setLoading(false);
      } catch (error) { console.error("[AppleCoreArtistProfile] Load error:", error); setLoading(false); }
    };
    loadData();
  }, [artistId]);
  ...
  const releasedReleases = useMemo(() => {
    const catalog = getVisibleReleasedReleases(releases, { platform: "applecore" });
    return catalog
  ...
  const totalStreams = releasedReleases.reduce((sum, r) => sum + (r.platform_streams?.AppleCore || 0), 0);
  ...
  const songPlatformStreams = useMemo(() => {
    ...
    popularSongs.forEach((song) => {
      ...
      if (rel?.platform_streams?.AppleCore) map[song.id] = rel.platform_streams.AppleCore;
    });
    return map;
  ...
  if (loading) return (<div className="flex min-h-[60vh] items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-rose-400/30 border-t-rose-400" /></div>);
  if (!profile) return (<div className="rounded-2xl border border-rose-200/10 bg-white/[0.03] px-4 py-6 text-center text-xs text-white/50">No AppleCore profile data found yet.</div>);

  return (
    <div className="relative pb-6 text-white">
      {/* ── Hero ── */}
      <div className="relative h-72 w-full overflow-hidden">
        {profile?.applecore_header_image ? (
          <img src={profile.applecore_header_image} alt={profile.artist_name} className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full items-center justify-center bg-gradient-to-br from-rose-600/50 via-pink-500/30 to-black text-4xl font-bold text-white/30">{profile?.artist_name?.[0] || "?"}</div>
        )}
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <button className="h-8 w-8 flex items-center justify-center text-white/60 hover:text-rose-300"><Shuffle className="h-5 w-5" /></button>
        <button className="flex h-12 w-12 items-center justify-center rounded-full bg-rose-500 shadow-lg shadow-rose-500/30 hover:bg-rose-400 hover:scale-105 transition-all"><Play className="h-5 w-5 fill-white text-white ml-0.5" /></button>
      </div>
      ...
      {artistPick && (
        <div className="mx-4 mb-4 flex items-center gap-3 rounded-lg bg-white/[0.06] p-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-white/50">Editor's Pick</p>
            <p className="text-sm font-semibold text-white truncate">{artistPick.release_name}</p>
            <p className="text-[10px] text-white/40">{formatStreams(artistPick.platform_streams?.AppleCore || artistPick.lifetime_streams || 0)}</p>
          </div>
      ...
      <div className="flex gap-4 border-b border-white/[0.08] px-4">
        <button onClick={() => setActiveTab("music")} className={`pb-2.5 text-sm font-semibold transition ${activeTab === "music" ? "text-white border-b-2 border-rose-400" : "text-white/50"}`}>Music</button>
        <button onClick={() => setActiveTab("events")} className={`pb-2.5 text-sm font-semibold transition ${activeTab === "events" ? "text-white border-b-2 border-rose-400" : "text-white/50"}`}>Events</button>
      </div>
      ...
      {activeTab === "music" ? (
        <div className="space-y-6 pb-6 pt-4">
          ...
          <section>
            <div className="flex items-center justify-between px-4 mb-3">
              ...
              <button onClick={() => setShowAll(true)} className="text-xs text-white/50 hover:text-white">Show all</button>
            </div>
          ...
          {/* ── Engagement Panel ── */}
          {engagementRelease && (
            <section className="mx-4 rounded-xl border border-rose-500/20 bg-rose-500/[0.04] p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2"><BarChart3 className="w-4 h-4 text-rose-400" /><h3 className="text-sm font-semibold">{engagementRelease.release_name}</h3></div>
              ...
              <FanEngagementPanel release={engagementRelease} hype={profile?.hype || 50} followers={profile?.followers || 1000} platformTheme="rose" />
            </section>
          )}

          {/* ── Discography ── */}
          <DiscographySection releases={releasedReleases} accentColor="rose" />
          ...
          <section className="mx-4 rounded-xl bg-white/[0.04] p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowProfileEditor(true)} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors text-rose-400" title="Edit profile">
                <Edit3 className="w-4 h-4" />
              </button>
            </div>
            <div className="flex items-start gap-3">
              <div className="h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg bg-white/5">
                {profile?.applecore_profile_image || profile?.artist_image ? (
                  <img src={profile?.applecore_profile_image || profile?.artist_image} alt={profile.artist_name} className="h-full w-full object-cover" />
                ) : (
              ...
              <div className="flex-1">
                <p className="text-xs leading-relaxed text-white/60">
                  {profile?.applecore_bio || profile?.about_text || profile?.bio ||
                    `${profile?.artist_name || "This artist"} delivers ${profile?.genre || "premium"} sounds with editorial precision. Based in ${profile?.region || "your region"}, their catalog is built for hi-res listening, curated playlists, and critical acclaim.`}
                </p>
                <div className="grid grid-cols-2 gap-3 pt-3">
                  <div className="rounded-lg border border-rose-500/10 bg-white/[0.04] p-3">
                    <p className="text-[10px] text-white/40">AppleCore ranking</p>
        ...
        <div className="px-4 pb-8 pt-5"><ArtistEventsTab artistId={profile?.id} events={events} platformAccent="rose" /></div>
      )}

      {/* ── Show All Modal ── */}
      {showAll && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 px-4 py-6" onClick={() => setShowAll(false)}>
          <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-2xl border border-rose-200/10 bg-[#111118] p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {RELEASE_FILTERS.map((filter) => (
                <button key={filter} onClick={() => setReleaseFilter(filter)} className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${releaseFilter === filter ? "bg-rose-500/20 text-white border border-rose-400/50" : "bg-white/5 text-white/50 border border-transparent hover:bg-white/10"}`}>{filter}</button>
              ))}
            </div>
              ...
              {filteredReleases.length === 0 ? (<p className="text-xs text-white/50 text-center py-6">No releases match this filter.</p>) : (
                filteredReleases.map((release) => (
                  <div key={release.id} className="flex items-center gap-3 rounded-lg p-2 hover:bg-white/[0.04] transition-colors min-h-[56px]">
                    ...
                    <span className="text-xs text-white/30">{formatStreams(release.platform_streams?.AppleCore || release.lifetime_streams || 0)}</span>
                  </div>
                ))
              )}
            </div>
