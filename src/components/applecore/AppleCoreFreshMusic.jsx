...
export default function AppleCoreFreshMusic({ releases, onReleaseClick }) {
  if (!releases || releases.length === 0) {
    return (
      <section className="space-y-3">
        <div className="px-4"><h2 className="text-white text-lg font-bold">New Releases</h2></div>
        <div className="text-white/40 text-xs text-center py-8 mx-4 rounded-lg border border-white/[0.06]">No new releases yet.</div>
      </section>
    );
  }
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 px-4" style={{ scrollbarWidth: "none", msOverflowStyle: "none", WebkitOverflowScrolling: "touch" }}>
        {releases.map((release) => (
          <button key={release.id} onClick={() => onReleaseClick?.(release)} className="flex-shrink-0 w-[140px] text-left">
            <div className="aspect-square w-full rounded-xl overflow-hidden bg-white/5 mb-2 group">
              {release.cover_artwork_url ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-rose-600/30 to-pink-500/10 flex items-center justify-center text-white/30 text-xs">No Art</div>
              )}
            </div>
