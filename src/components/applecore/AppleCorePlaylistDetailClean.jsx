...
export default function AppleCorePlaylistDetailClean({ playlist, onClose }) {
  ...
  const loadPlaylistSongs = async () => {
    try {
      ...
      const releasedReleases = getVisibleReleasedReleases(releases, { platform: "applecore" });
      ...
      const playlistReleases =
        matchedReleases.length > 0
          ? matchedReleases
          : getVisibleReleasedReleases(playlist?.releases || [], { platform: "applecore" });
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[420px]">
        <div className="relative min-h-[300px] bg-gradient-to-b from-rose-900/80 to-black">
          <button
            onClick={onClose}
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <div className="flex items-end gap-4">
              ...
              <div className="min-w-0 flex-1 pb-1">
                <p className="mb-1 text-xs font-semibold text-rose-200/80">Curated Playlist</p>
                ...
                <p className="mb-3 max-w-[22ch] text-sm leading-5 text-white/60 sm:max-w-[28ch]">
                  {playlist.description || "High-fidelity picks and editorial favorites from the AppleCore catalog."}
                </p>
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-white/80">
                  <span className="font-semibold text-rose-200">AppleCore</span>
        ...
        <div className="bg-gradient-to-b from-black/40 to-black px-6 py-4">
          <div className="flex items-center gap-4">
            <button className="flex h-14 w-14 items-center justify-center rounded-full bg-rose-500 transition hover:scale-105">
        ...
        <div className="px-6 pb-6">
          <div className="mb-4 flex items-center gap-4 border-b border-rose-200/10 pb-2 text-xs text-white/50">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-rose-300/20 border-t-rose-300" />
            </div>
          ) : songs.length === 0 ? (
            ...
            <div className="space-y-1">
              {songs.map((song, index) => (
                <div
                  key={song.id}
                  className="group flex items-center gap-3 rounded-xl px-2 py-2 hover:bg-rose-500/5"
                >
