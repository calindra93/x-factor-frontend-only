...
export default function SoundburstFreshMusic({ releases, onReleaseClick }) {
  if (!releases || releases.length === 0) {
    return (
      <section className="space-y-3">
        <div className="px-4">
          <h2 className="text-white text-lg font-bold">Fresh Drops</h2>
        </div>
        <div className="text-white/40 text-xs text-center py-8 mx-4 rounded-lg border border-white/[0.06]">
          No new underground drops yet.
        </div>
      </section>
    );
  }
          ...
          >
            <div className="aspect-square w-full rounded-md overflow-hidden bg-white/5 mb-2 group">
              {release.cover_artwork_url ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-emerald-600/30 to-cyan-500/10 flex items-center justify-center text-white/30 text-xs">No Art</div>
              )}
            </div>
