...
import { Music, Rocket, Clock, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { base44 } from "@/api/base44Client";
import { showToast } from "@/components/ui/toast-provider";

export default function RecordedSongs({ songs, profile, onRefresh, onReleaseSingle }) {
  ...
  const handleDelete = async (song) => {
    ...
    try {
      ...
      console.error("[RecordedSongs] Delete failed:", e);
  ...
  if (!songs || songs.length === 0) {
    return (
      <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-6 text-center">
        ...
        <p className="text-gray-500 text-sm">No recorded songs yet</p>
      </div>
    );
  }

  const handleRelease = (songId) => {
    const song = songs.find(s => s.id === songId);
    if (song && onReleaseSingle) {
      onReleaseSingle(song);
    }
  };
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {song.status === 'waiting_on_collab' && (
                <div className="flex items-center gap-1">
                  <Clock className="w-2.5 h-2.5 text-yellow-400" />
                  <span className="text-yellow-400 text-[9px]">Waiting on feature</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-1">
              <button
                ...
                title="Delete song"
              >
                <Trash2 className="w-2.5 h-2.5 text-gray-500" />
              </button>
              <Button
                size="sm"
                onClick={() => handleRelease(song.id)}
                disabled={song.status === 'waiting_on_collab' || !onReleaseSingle}
                className="bg-red-600 hover:bg-red-500 text-white rounded h-6 text-[10px] px-2 disabled:opacity-30"
              >
                <Rocket className="w-2.5 h-2.5 mr-1" />
                {song.status === 'waiting_on_collab' ? 'Pending' : 'Release'}
              </Button>
            </div>
          </div>
        );
