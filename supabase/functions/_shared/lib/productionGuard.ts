 ...
 */
export function shouldBlockHosted(url: string): boolean {
  ...
  const isCI = (typeof process !== 'undefined' && (process.env?.CI === 'true' || process.env?.GITHUB_ACTIONS === 'true')) ||
                (typeof globalThis !== 'undefined' && (globalThis as any)?.Deno?.env?.get('CI') === 'true') ||
                (typeof globalThis !== 'undefined' && (globalThis as any)?.Deno?.env?.get('GITHUB_ACTIONS') === 'true');
  const allowHosted = (typeof process !== 'undefined' && process.env?.ALLOW_HOSTED_SUPABASE === '1') ||
                     (typeof globalThis !== 'undefined' && (globalThis as any)?.Deno?.env?.get('ALLOW_HOSTED_SUPABASE') === '1');
  
  return isHosted && !isCI && !allowHosted;
}

export function throwIfProductionInTest(supabaseUrl: string, context: string = 'Supabase client'): void {
  // Check if we're in a test environment
  const isTestEnvironment = 
    (typeof process !== 'undefined' && (process.env?.NODE_ENV === 'test' || process.env?.VITEST === '1')) ||
    (typeof globalThis !== 'undefined' && (globalThis as any)?.Deno?.env?.get('VITEST') === '1') ||
    (typeof globalThis !== 'undefined' && (globalThis as any)?.Deno?.env?.get('NODE_ENV') === 'test');
  ...
  const isProductionSupabase = isHostedSupabaseUrl(supabaseUrl);

  if (isProductionSupabase) {
    throw new Error(
      `🚨 PRODUCTION GUARD: ${context} attempted to connect to hosted Supabase in test environment!\n` +
      `   URL: ${supabaseUrl}\n` +
      `   Environment: NODE_ENV=${process?.env?.NODE_ENV}, VITEST=${process?.env?.VITEST}\n` +
...
export function throwIfHostedSupabase(supabaseUrl: string, context: string = 'Supabase client'): void {
  if (!supabaseUrl) {
    return; // No URL provided, will use fallback
  }
